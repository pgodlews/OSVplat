...
| `sfm` | `30_run_sfm.py` + `32_pick_model.py` | `88_fisheye_sfm.py`: one rig pass inside an 88° circle, `32_pick_model.py`, refined calibration, angular error, with `sfm.upright` the model levelled and, with GPS, put in metres (`upright.py`), and a dataset with flattened names and valid-circle masks |
...
**Cache keys.** `JobConfig.k_frames` hashes a `FISHEYE_PIPELINE` term for `.OSV` input only, so every fisheye key differs from every stitched key, and every stitched job keeps the key it was cached under (`test_stages.py` pins both). Bump `FISHEYE_PIPELINE` after changing a fisheye script to invalidate fisheye caches alone. A change to the fisheye reconstruction alone bumps `FISHEYE_SFM` instead, which `k_sfm` hashes for `.OSV` input only: sfm, train and export re-run, frames, selection and masks stay cached.
...
**Upright and in metres, `sfm.upright`.** Raw `.OSV` only, refused for stitched input. On by default: a request that leaves it out gets `UPRIGHT_DEFAULT` (True) for `.OSV` input (`main._prepare`), and the UI checkbox follows `recommended_upright` in `/api/inputs`. The sfm stage adds `--upright <clip> --selection select/selection.json` (and `--start` for a trimmed clip) to `88_fisheye_sfm.py`. It levels the model with the clip's orientation stream, and scales it to metres and turns it to north when the clip carries usable GPS. When a step cannot be trusted the script falls back: to levelled without scale, or to the model as SfM made it. It records the outcome in `summary.json` under `alignment` either way, so the stage caches a fallback, with a warning. A summary with no `alignment` at all is refused, because the key says one was attempted. `JobConfig.k_sfm` hashes an `UPRIGHT` term only while the option is on (the field itself is kept out of the sfm dump), so a job with it off keeps its old key; bump `UPRIGHT` after changing what `upright.py` does to a model. Frames, selection and masks are unaffected. What it does and the fallbacks: docs/how-it-works.md, "Upright and in metres".
